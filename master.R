...
foo=read.csv("./tensio_data/CR1000-AB2_Table1.csv")
...
for(p in 1:5) { #loop through 5 plots
  ...
  e=eid[1]
    ...
    for(t in tid[1:2]) { 
      ee=e
	  if(e>=11)ee=e+1
	  tensiofile=paste("tensio_", p,"_",ee,"_",t,sep="")
	  f_ts_cal=lappend(f_ts_cal,tensiofile)
...
#print(f_ts_cal)
outte=file("master.sh")
prebuf="#!/bin/bash\ncd talog"
prp=paste("parrep to_precal3.par to_precal3.pst c_",f_ts_cal,".pst",sep="")
ffs=paste("./run_pest2.sh c_",f_ts_cal," ",f_ts_cal,sep="")

writeLines(c(prebuf,prp,ffs),outte)
#for(l in f_ts_cal) {
#	write(paste("./talog/run_pest2.sh to_precal3 ",l,sep=""),file=outte,append=TRUE,ncolumns=1)
#}
close(outte)
